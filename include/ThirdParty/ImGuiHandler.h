...
namespace ImGuiHandler
{
    ...
    inline void DrawSpotLightsGui(std::vector<std::shared_ptr<SpotLight>>& lights, int maxCount)
    {
        ...
        for (int i = 0; i < current; )
        {
            ...
            if (open)
            {
                ...
                if (ImGui::Button("Normalize Dir")) {
                    const float len2 = props.Direction.x * props.Direction.x
                                     + props.Direction.y * props.Direction.y
                                     + props.Direction.z * props.Direction.z;
                    if (len2 > 1e-12f) {
                        const float invLen = 1.0f / std::sqrt(len2);
                        props.Direction *= invLen;
                    } else {
                        props.Direction = glm::vec3(0.f, -1.f, 0.f);
                    }
                }
